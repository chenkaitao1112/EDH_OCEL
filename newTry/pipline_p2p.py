...
def process_object_id(row):
    ...
    if row['object_type'] in keep_types:
        ...
        return "Other"
def process_vmap_lifecycle(df):
    # 删除原omap列
    df = df.drop(columns=['vmap_lifecycle'])
    return df
...
if __name__ == "__main__":
    ...
    for eventlog in list_eventlog:
        ...
        df_events_update = process_omap(df_events_ab)
        df_events_updated = process_vmap_lifecycle(df_events_update)
